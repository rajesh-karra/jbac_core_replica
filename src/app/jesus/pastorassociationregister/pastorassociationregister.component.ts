...
@Component({
  selector: 'app-pastorassociationregister',
  templateUrl: './pastorassociationregister.component.html',
  styleUrls: ['./pastorassociationregister.component.css']
})
export class PastorassociationregisterComponent {
  ...
  constructor(private formBuilder: FormBuilder, private service: ServiceService, private router: Router, private modalService: NgbModal,
    ...
    this.pastorsassociations = this.formBuilder.group({
      ...
      pa_president: [''],
      std_phonenumber: [''],
      totalleadrs: ['', [Validators.required]],
      address: [''],
      district_id: ['', [Validators.required]],
      constituency_id: ['', [Validators.required]],
      mandal_id: ['', [Validators.required]],
  ...
  addNew(event:any,service:any) {
    if (event.target.value == 1) {
      this.modalService.open(service, { size: 'xl', centered: true });
    }
  }
  ProofValue: any;
  addNewSubmit() {
