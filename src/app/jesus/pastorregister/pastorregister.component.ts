...
@Component({
  selector: 'app-pastorregister', 
  templateUrl: './pastorregister.component.html',
  styleUrls: ['./pastorregister.component.css'],
  
  
})
export class PastorregisterComponent {
godOptions: string[] = [
    'ప్రార్ధన చేస్తాను',
    'డబ్బులు ఇవ్వగలను',
    'నా టాలెంట్ ఉపయోగించ గలను',
    'సమయం ఇవ్వగలను',
    'క్రైస్తవులకు అన్యాయం జరిగితే వాయిస్ ఇస్తాను',
    'శ్రమదానం చేస్తాను'
  ];
 ...
 onGodCheckboxChange(event: any) {
    const god: FormArray = this.pastorform.get('god') as FormArray;

    if (event.target.checked) {
      god.push(this.formBuilder.control(event.target.value));
    } else {
      const index = god.controls.findIndex(x => x.value === event.target.value);
      god.removeAt(index);
    }
  }

  onSubmit() {
    console.log(this.pastorform.value);
    // Example output:
    // { god: ['ప్రార్ధన చేస్తాను', 'పని చేస్తాను'] }
  }
