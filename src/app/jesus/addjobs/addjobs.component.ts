...
export class AddjobsComponent {
  ...
  postchurchtimings() {
    ...
    if (this.addingchurchtimimngs.invalid) {
      Swal.fire('* మార్క్ వున్నా ఫీల్డ్స్ అన్ని తప్పనిసరిగా ఫిల్ చేయాలి');
    } else {
      var data = {
  ...
  postjobs() {
    ...
    if (this.jobsadding.invalid) {
      ...
      var data = {
        ...
        experience_t: this.jobsadding.value.experience_t,
      ...
      this.service.postjobs(data).subscribe((res: any) => {
        if (res.status == 200) {
          Swal.fire('విజయవంతముగా సబ్మిట్ చేసినారు ')
  ...
  getmandals(event: any) {
    ...
    this.service.getmandals().subscribe(res => {
      ...
      Swal.fire(' దయచేసి మరల ప్రయత్నించండి.');
      this.showSpinner = false; 
